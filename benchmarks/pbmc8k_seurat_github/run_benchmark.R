...
PBMC8K_DATA_URL <- "https://cf.10xgenomics.com/samples/cell/pbmc8k/pbmc8k_filtered_gene_bc_matrices.tar.gz"
...
pbmc_archive <- file.path(data_root, "pbmc8k_filtered_gene_bc_matrices.tar.gz")

find_matrix_dir <- function(root) {
  candidates <- list.files(
    file.path(root, "filtered_gene_bc_matrices"),
    full.names = TRUE,
    recursive = FALSE
  )
  for (candidate in candidates) {
    matrix_file <- file.path(candidate, "matrix.mtx")
...
download_pbmc8k <- function() {
  existing <- find_matrix_dir(data_root)
  if (!is.null(existing)) {
    return(invisible(existing))
  }

  message("==> Downloading PBMC 8K data from 10x Genomics...")
  utils::download.file(PBMC8K_DATA_URL, pbmc_archive, mode = "wb", quiet = FALSE)
  utils::untar(pbmc_archive, exdir = data_root)

  matrix_dir <- find_matrix_dir(data_root)
  if (is.null(matrix_dir)) {
    stop("PBMC 8K archive did not contain a filtered gene-barcode matrix directory.")
  }
  invisible(matrix_dir)
}
...
run_workflow <- function(backend, counts, output_file) {
  ...
  step <- timed_step("04_scale_data", {
    mat <- GetAssayData(seu, assay = "RNA", layer = "data")[hvf, , drop = FALSE]
...
cat("==> Related R project: ", TENXPBMC_DATA_URL, "\n", sep = "")
cat("==> Data source: ", PBMC8K_DATA_URL, "\n\n", sep = "")

pbmc_matrix_dir <- download_pbmc8k()
