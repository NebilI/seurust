...
test_that("fast_dist accepts integer and double neighbor ids", {
  ...
  got_int <- seurust::fast_dist(x, y, n_int)
  got_dbl <- seurust::fast_dist(x, y, n_dbl)
  expect_identical(got_int, got_dbl)
  expect_named(got_int, names(n_int))
  expect_equal(got_int[[2]], sqrt(colSums((t(y[n_int[[2]], ]) - x[2, ])^2)))

  expect_identical(seurust::fast_dist(x, y, n_int[1:3]), list())
  expect_error(seurust::fast_dist(x, y, c(list(16L), n_int[-1])), "outside")
  expect_error(seurust::fast_dist(x, y, c(list(NA_integer_), n_int[-1])), "outside")
})
...
test_that("fast_dist matches Seurat for integer and double ids", {
  ...
  expect_equal(seurust::fast_dist(x, y, n_int), seurat_fast_dist(x, y, n_int), tolerance = 1e-12)
  n_dbl <- lapply(n_int, as.double)
  expect_equal(seurust::fast_dist(x, y, n_dbl), seurat_fast_dist(x, y, n_dbl), tolerance = 1e-12)
})
...
test_that("GraphToNeighborHelper reads neighbors from rows", {
  ...
  out <- seurust::GraphToNeighborHelper(g)
  ...
  expect_error(seurust::GraphToNeighborHelper(ragged), "equal number of neighbors")
})
...
test_that("GraphToNeighborHelper matches Seurat on an asymmetric kNN graph", {
  ...
  expect_identical(seurust::GraphToNeighborHelper(g), seurat_helper(g))
})
...
test_that("RowMergeMatrices accepts both sparse layouts", {
  ...
  from_csc <- seurust::RowMergeMatrices(m1, m2, rn1, rn2, all_rn)
  from_csr <- seurust::RowMergeMatrices(
...
test_that("DirectSNNToFile returns a dgCMatrix and writes the edge file", {
  ...
  on.exit(unlink(path))
  snn <- seurust::DirectSNNToFile(nn, prune = 1 / 15, display_progress = FALSE, filename = path)
  expect_s4_class(snn, "dgCMatrix")
  expect_identical(snn, seurust::ComputeSNN(nn, prune = 1 / 15))
  ...
  expect_identical(seurust::ComputeSNN(nn, prune = 1 / 15), snn)
})
...
test_that("DirectSNNToFile and WriteEdgeFile match Seurat byte-for-byte", {
  ...
  got <- seurust::DirectSNNToFile(nn, 1 / 15, FALSE, f_rust)
  expect_equal(got, expected, tolerance = 1e-12)
  expect_identical(readLines(f_rust), readLines(f_seurat))

  seurust::WriteEdgeFile(expected, f_rust, FALSE)
  expect_identical(readLines(f_rust), readLines(f_seurat))
})
...
test_that("dense kernels match base R", {
  ...
  expect_equal(seurust::FastCov(a), cov(a), tolerance = 1e-12)
  expect_true(isSymmetric(seurust::FastCov(a)))
  expect_equal(seurust::FastCov(a, center = FALSE), crossprod(a) / 119, tolerance = 1e-12)
  expect_equal(seurust::FastCovMats(a, b), cov(a, b), tolerance = 1e-12)
  expect_identical(seurust::FastRBind(a, a[1:5, ]), rbind(a, a[1:5, ]))
  expect_error(seurust::FastRBind(a, b), "columns")
  expect_error(seurust::FastCovMats(a, b[1:10, ]), "rows")

  ai <- matrix(sample.int(9, 60, replace = TRUE), nrow = 12)
  expect_equal(seurust::FastCov(ai), cov(ai), tolerance = 1e-12)
  expect_equal(seurust::RowVar(ai), apply(ai, 1, var), tolerance = 1e-12)
})
...
test_that("dense kernels match Seurat", {
  ...
  expect_equal(seurust::FastCov(a), seurat_cov(a), tolerance = 1e-12)
  expect_equal(seurust::FastCovMats(a, b), seurat_cov_mats(a, b), tolerance = 1e-12)
  expect_identical(seurust::FastRBind(a, a), seurat_rbind(a, a))
})
...
test_that("numeric vector arguments accept integers", {
  ...
  expect_equal(seurust::SparseRowVar2(mat, mu, FALSE), seurust::SparseRowVar2(mat, as.double(mu), FALSE))
})
